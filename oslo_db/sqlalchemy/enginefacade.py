...
class _TransactionFactory(object):
    ...
    :class:`._TestTransactionFactory` subclass used by the oslo.db_2 test suite.
    ...
    def __init__(self):
        ...
        # other options that are defined in oslo.db_2.options.database_opts
...
class _TestTransactionFactory(_TransactionFactory):
    ...
    Note that while this is used by oslo.db_2's own tests of
            ...
            try:
                if self.session is not None:
                    ...
                    try:
                        ...
                        # raw "raise" in many other places in oslo.db_2 already
                        # (and one six.reraise()).
                        raise
                    finally:
                        self.transaction = None
...
class LegacyEngineFacade(object):
    """A helper class for removing of global engine instances from oslo.db_2.

    .. deprecated:: 1.12.0
        Please use :mod:`oslo_db.sqlalchemy.enginefacade` for new development.

    As a library, oslo.db_2 can't decide where to store/when to create engine
    and sessionmaker instances, so this must be left for a target application.

    On the other hand, in order to simplify the adoption of oslo.db_2 changes,
    ...
    :keyword max_retries: maximum db_2 connection retries during startup.
