...
def _db_dupe_key_error(integrity_error, match, engine_name, is_disconnect):
...
def _raise_savepoints_as_dberrors(error, match, engine_name, is_disconnect):
    ...
    # to be an InternalError. It's expected to be wrapped into oslo.db_2 error.
...
ROLLBACK_CAUSE_KEY = 'oslo.db_2.sp_rollback_cause'
