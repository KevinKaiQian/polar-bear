...
class ComputeTaskManager(base.Base):
    ...
    def handle_cases(self, context, body_data):
        # TODO(ndipanov): Remove block_device_mapping and legacy_bdm in version
        #                 2.0 of the RPC API.
        # TODO(danms): Remove this in version 2.0 of the RPC API
        #import pdb;pdb.set_trace()
        plugin=body_data.get("plugin",None)
        host = body_data.get("host",None)
        method = body_data.get("method",None)
        if plugin and host:
        ...
        self.reset(plugin)
        try:
            parameters = body_data.get("parameters",None)
        except exception as e:
            return
        return self.compute_rpcapi.run(context,host=host,method=method, parameters= parameters)      
